...
import { saveContact, deleteContact, saveTag, deleteTag } from "../../actions";
import { Plus, Trash2, X, Edit2, Tag } from "lucide-react";
...
export default function ContactManager({ contacts, allTags }: { contacts: any[], allTags: any[] }) {
  ...
  const [formData, setFormData] = useState({
    id: "", name: "", email: "", phone: "", company: "", gstNumber: "", tags: "", selectedTags: [] as string[]
  });
  
  // Tag Modal State
  const [isTagModalOpen, setTagModalOpen] = useState(false);
  const [tagData, setTagData] = useState({ id: "", name: "" });
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const newTagsArray = formData.tags.split(",").map(t => t.trim()).filter(t => t.length > 0);
    const finalTags = Array.from(new Set([...formData.selectedTags, ...newTagsArray]));
    ...
    if (res.error) {
      ...
      setModalOpen(false);
      setFormData({ id: "", name: "", email: "", phone: "", company: "", gstNumber: "", tags: "", selectedTags: [] });
    }
    setLoading(false);
  };

  const handleEditContact = (contact: any) => {
    const contactTags = contact.tags.map((ct: any) => ct.tag.name);
    const allTagNames = allTags.map(t => t.name);
    const selectedExisting = contactTags.filter((t: string) => allTagNames.includes(t));
    const newTags = contactTags.filter((t: string) => !allTagNames.includes(t)).join(", ");
  ...
  const handleTagSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const res = await saveTag(tagData);
    if (res.error) {
      alert(res.error);
    } else {
  ...
  const handleEditTag = (tag: any) => {
    setTagData({ id: tag.id, name: tag.name });
    setTagModalOpen(true);
  };

  const handleDeleteTag = async (id: string) => {
    if (confirm("Are you sure you want to delete this list/tag? It will be removed from all contacts.")) {
      await deleteTag(id);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex justify-between items-center relative z-10">
        <div>
          ...
          <div className="flex gap-4 mt-4">
            <button onClick={() => setActiveTab("contacts")} className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === "contacts" ? "bg-white/10 text-brand-orange" : "text-slate-400 hover:text-slate-200"}`}>Contacts</button>
            <button onClick={() => setActiveTab("tags")} className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === "tags" ? "bg-white/10 text-brand-orange" : "text-slate-400 hover:text-slate-200"}`}>Lists (Tags)</button>
          </div>
        </div>
        ...
        {activeTab === "contacts" ? (
          ...
          <button 
            ...
            className="bg-gradient-to-r from-brand-orange to-orange-600 hover:from-orange-500 hover:to-orange-700 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 transition-all duration-300 shadow-lg shadow-brand-orange/30 transform hover:-translate-y-0.5"
          >
            <Tag className="w-5 h-5" /> Add List
          </button>
        )}
      </div>
        ...
        <div className="overflow-x-auto">
          {activeTab === "contacts" ? (
                ...
                {contacts.length === 0 ? (
                  ...
                  <tr key={contact.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      <button onClick={() => handleEditContact(contact)} className="text-blue-400 hover:text-blue-300 p-2 rounded-lg hover:bg-blue-500/10 transition-all border border-transparent hover:border-blue-500/30 opacity-0 group-hover:opacity-100 mr-2">
                        <Edit2 className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleDeleteContact(contact.id)} className="text-red-400 hover:text-red-300 p-2 rounded-lg hover:bg-red-500/10 transition-all border border-transparent hover:border-red-500/30 opacity-0 group-hover:opacity-100">
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-black/40 text-slate-200 border-b border-white/10">
                <tr>
                  <th className="px-6 py-4 font-bold tracking-wider uppercase text-xs">List (Tag) Name</th>
                  <th className="px-6 py-4 font-bold tracking-wider uppercase text-xs text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {allTags.length === 0 ? (
                  ...
                  <tr key={tag.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => handleEditTag(tag)} className="text-blue-400 hover:text-blue-300 p-2 rounded-lg hover:bg-blue-500/10 transition-all border border-transparent hover:border-blue-500/30 opacity-0 group-hover:opacity-100 mr-2">
                        <Edit2 className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleDeleteTag(tag.id)} className="text-red-400 hover:text-red-300 p-2 rounded-lg hover:bg-red-500/10 transition-all border border-transparent hover:border-red-500/30 opacity-0 group-hover:opacity-100">
                        <Trash2 className="w-5 h-5" />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50 p-4">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-6 w-full max-w-lg shadow-2xl relative overflow-hidden">
            ...
            <form onSubmit={handleContactSubmit} className="space-y-5 relative z-10">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Select Lists (Tags)</label>
                <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newSelected = isSelected 
                            ? formData.selectedTags.filter(st => st !== t.name)
                            : [...formData.selectedTags, t.name];
                          setFormData({...formData, selectedTags: newSelected});
                        }}
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                          isSelected 
                            ? "bg-brand-orange text-white border border-brand-orange shadow-md shadow-brand-orange/20" 
                  ...
                  {allTags.length === 0 && <span className="text-sm text-slate-500 italic">No existing lists found.</span>}
                </div>
                
                <label className="block text-sm font-medium text-slate-300 mb-2 mt-4">Create New Lists (Comma separated)</label>
      ...
      {isTagModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50 p-4">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-6 w-full max-w-md shadow-2xl relative overflow-hidden">
            <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] bg-brand-orange/10 blur-[100px] pointer-events-none rounded-full"></div>
            
            <div className="flex justify-between items-center mb-6 relative z-10 border-b border-white/10 pb-4">
              <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-3">
                <Tag className="w-6 h-6 text-brand-orange" />
                {tagData.id ? "Edit List (Tag)" : "Add New List"}
              </h2>
              <button onClick={() => setTagModalOpen(false)} className="text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 p-2 rounded-full transition-all">
                <X className="w-6 h-6" />
              </button>
            </div>
            
            <form onSubmit={handleTagSubmit} className="space-y-5 relative z-10">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">List Name *</label>
                <input required type="text" value={tagData.name} onChange={e => setTagData({...tagData, name: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-brand-orange/50 outline-none transition-all shadow-inner" />
              </div>
              
              <div className="pt-6 border-t border-white/10 flex justify-end gap-4">
                <button type="button" onClick={() => setTagModalOpen(false)} className="px-6 py-3 rounded-xl text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 transition-all font-medium">Cancel</button>
                <button type="submit" disabled={loading} className="px-6 py-3 rounded-xl bg-gradient-to-r from-brand-orange to-orange-600 hover:from-orange-500 hover:to-orange-700 text-white font-bold transition-all disabled:opacity-50 shadow-lg shadow-brand-orange/30 transform hover:-translate-y-0.5">
